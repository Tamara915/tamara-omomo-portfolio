/* ============================================================
   interactions.js — kinetic / interactive type layer
...
(function () {
  ...
  /* ---- hero: variable-weight wave follows cursor ---- */
  function heroType() {
    const h1 = $('#heroName');
    if (!h1) return;
    ...
    // rAF-driven entrance (staggered weight-bloom + fade in)
    if (!reduced) {
      ...
      // safety net: guarantee final state even if rAF is throttled
    ...
    function frame() {
      for (const p of pts) {
        ...
        const w = Math.round(lerp(400, 700, ease));
  ...
  function scramble(el, opts = {}) {
    ...
    const queue = finalText.split('').map(ch => ({
      ch,
      from: Math.floor(Math.random() * dur * 0.4),
      to: Math.floor(dur * 0.4 + Math.random() * dur * 0.6)
    }));
    function tick(now) {
  ...
  // hover-decode for nav links and labeled elements
  $$('.dx').forEach(el => {
  ...
  /* ---- count-up numbers ---- */
  function countUp(el) {
    if (reduced) { el.textContent = el.dataset.prefix + el.dataset.count + (el.dataset.suffix || ''); return; }
  ...
  /* ---- marquee: rAF-driven translateX + velocity skew ---- */
  (function marquee() {
  ...
  if (finePointer && !reduced) {
    ...
    // start off-screen so no stray dot before first mouse move
